      ...
      <div class="container-fluid">
        <div class="justify-content-md-center">
          ...
          <div class="container text-center ">
            ...
            @endif
            <div class="mr-5 w-25 m-2">
              <form class="d-flex mr-2" action="{{ '/search-patient/' . $lid }}" method="get">
                @csrf
                {{ method_field('PUT') }}
                <input class="form-control mr-2 text-right" id="search" type="search" name="search" placeholder="ابحث" aria-label="Search" tabindex="1">
            ...
            <table class="table table-responsive">
              <thead class="table-dark">
                <tr>
                  ...
                  <th scope="col">عنوان المريض</th>
                  <th scope="col">رقم هوية المريض</th>
                  <th scope="col">تاريخ الميلاد</th>
                  <th scope="col">جنس المريض</th>
                  <th scope="col">رقم الهاتف</th>
                  <th scope="col"> تعديل البيانات</th>
                  <th scope="col">عرض الفحوصات</th>
                </tr>
              </thead>
              <tbody>
                @foreach ($patients as $key => $patient)
                  <tr>
                    ...
                    <td>{{ $patient->PID }}</td>
                    <td>{{ $patient->PDofB }}</td>
                    @if ($patient->PGender == 'male')
                      <td>ذكر</td>
                    @else<td>انثى</td>
                    @endif
                    <td>{{ $patient->PPhone }}</td>
                    <td><a href="{{ '/patient/' . $patient->id . '/edit/' . $lid }}" class="btn btn-primary" tabindex="{{ $key + 3 }}">تعديل</a></td>

                    <td><a href="{{ '/patient-examination/' . $patient->id . '/' . $lid }}" class="btn btn-warning" tabindex="{{ $key + 4 }}">عرض</a></td>
